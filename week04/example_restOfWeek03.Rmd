...
plotSignalTracks(list(alignment="aligned/Myc.bam", peaks=peaks), region="numb", ensdb=ensdb, transcripts="full")
```
...
plotSignalTracks(c("aligned/Myc.bam"), region="numb", type = "alignment", ensdb=ensdb, transcripts="full")
```
...
```{r}
plotSignalTracks(list(full="tracks/Myc_full.bw", binned="tracks/Myc.bw", peaks=peaks), region="numb", ensdb=ensdb)
```
...
plotEnrichedHeatmaps(lapply(peakSignals, FUN=log1p))
```

# How many peaks overlap
...
head(proms)
peakOverlapsTSS <- overlapsAny(peaks, proms)
head(peakOverlapsTSS)
length(peakOverlapsTSS)
length(peaks)
length(proms)
table(peakOverlapsTSS)
```
...
```{r}
library(ChIPseeker)
pa <- ChIPseeker::annotatePeak(peaks, TxDb=ensdb)
plotAnnoPie(pa)
```
